...
app.controller('IdentityCtl', ['$scope', '$stateParams', 'IdentityService',
    ...
    function ($scope, $stateParams, IdentityService, ngDialog,
              FlowServiceV1,FlowServiceV2, DegradeService, AuthorityRuleService, ParamFlowService, MachineService, $interval, $location, $timeout) {
        ...
        $scope.addNewFlowNacosRule = function (resource) {
            if (!$scope.macInputModel) {
                return;
            }
            var mac = $scope.macInputModel.split(':');
            ...
            flowRuleDialogScope.saveRuleAndContinue = saveFlowRuleNacosAndContinue;
            flowRuleDialogScope.onOpenAdvanceClick = function () {
                flowRuleDialogScope.flowRuleDialog.showAdvanceButton = false;
            };
            flowRuleDialogScope.onCloseAdvanceClick = function () {
        ...
        function saveFlowRule() {
            if (!FlowServiceV1.checkRuleValid(flowRuleDialogScope.currentRule)) {
                return;
            }
            FlowServiceV1.newRule(flowRuleDialogScope.currentRule).success(function (data) {
                if (data.code === 0) {
        ...
        function saveFlowRuleNacosAndContinue() {
            if (!FlowServiceV2.checkRuleValid(flowRuleDialogScope.currentRule)) {
                return;
            }
            FlowServiceV2.newRule(flowRuleDialogScope.currentRule).success(function (data) {
